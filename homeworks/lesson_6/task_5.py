...
и метод draw (отрисовка). Метод выводит сообщение “Запуск отрисовки.” Создать три дочерних класса Pen (ручка),
 ...
 из классов методы должен выводить уникальное сообщение. Создать экземпляры классов
...
class Pen(Stationery):
    def draw(self):
        print('Дочерний класс ручка')
class Pencil(Stationery):
    def draw(self):
        print('Дочерний класс  карандаш')
class Handle(Stationery):
    def draw(self):
        print('Дочерний класс маркер')
s = Stationery()
s.draw()
p = Pen()
p.draw()
